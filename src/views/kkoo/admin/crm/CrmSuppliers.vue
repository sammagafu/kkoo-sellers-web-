<template>
  <VerticalLayout>
    <b-card title="Suppliers">
      <p class="text-muted mb-3">Supplier contacts: name, phone, email, address. Link purchase orders to suppliers.</p>
      ...
      <b-table v-if="items.length" :items="items" :fields="fields" striped responsive>
        <template #cell(actions)="data">
          <b-button size="sm" variant="outline-primary" :to="{ name: 'admin.crm.suppliers.detail', params: { id: String(data.item.id) } }">View</b-button>
        </template>
      </b-table>
...
const {
  ...
  setPage,
  load,
} = useCrmList((params) => crmApi.getSuppliers(params), {})

<template>
  <VerticalLayout>
    <b-card title="Products (Inventory)">
      <p class="text-muted mb-3">CRM inventory per business: name, SKU, cost, selling price, stock, reorder level. Low-stock filter available.</p>
      <div class="d-flex flex-wrap gap-2 mb-3">
        ...
        <b-button variant="primary" size="sm" :to="{ name: 'admin.crm.products.create' }">Add Product</b-button>
      </div>
      ...
      <b-table v-if="items.length" :items="items" :fields="fields" striped responsive>
        ...
        <template #cell(actions)="data">
          <b-button size="sm" variant="outline-primary" :to="{ name: 'admin.crm.products.detail', params: { id: String(data.item.id) } }">View</b-button>
        </template>
      </b-table>
      <p v-else-if="loading">Loading…</p>
      <EmptyState v-else title="No products" message="Add products (inventory) in CRM when the API is available. CRM is configured by your platform administrator or in the backend." />
      <b-pagination
        v-if="hasPagination"
        :model-value="page"
        :total-rows="total"

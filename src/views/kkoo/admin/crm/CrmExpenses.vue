<template>
  <VerticalLayout>
    <b-card title="Expenses">
      <p class="text-muted mb-3">Record expenses by category (rent, utilities, transport, supplies). Daily/monthly summaries available from reports.</p>
      ...
      <EmptyState v-else title="No expenses" message="Expenses will appear when the CRM API is configured. CRM is configured by your platform administrator or in the backend." />
      <b-pagination
        v-if="hasPagination"
        :model-value="page"
        :total-rows="total"
    ...
    <b-modal v-model="showAddModal" title="Add Expense" @hide="resetAddForm" @ok="onAddOk">
      ...
      <b-form>
        <b-form-group label="Category" label-for="add-category" :invalid-feedback="addErrors.category">
          <b-form-input id="add-category" v-model="addForm.category" placeholder="e.g. rent, utilities" />
        </b-form-group>
        <b-form-group label="Amount" label-for="add-amount" :invalid-feedback="addErrors.amount">
          <b-form-input id="add-amount" v-model.number="addForm.amount" type="number" step="0.01" placeholder="0" />
        </b-form-group>
        <b-form-group label="Description" label-for="add-description">
          <b-form-input id="add-description" v-model="addForm.description" placeholder="Optional" />
        </b-form-group>
        <b-form-group label="Date" label-for="add-date" :invalid-feedback="addErrors.date">
          <b-form-input id="add-date" v-model="addForm.date" type="date" />
        </b-form-group>
      </b-form>
    </b-modal>
  </VerticalLayout>
...
const {
  ...
  (params) => crmApi.getExpenses({ ...params, category: categoryFilter.value?.trim() || undefined }),
  {}
)

<template>
  <VerticalLayout>
    <b-button variant="outline-secondary" size="sm" class="mb-3" :to="{ name: 'admin.crm.invoices' }">← Back to Invoices</b-button>
    <b-card title="Create invoice">
      ...
      <b-form>
        ...
        <div class="d-flex gap-2">
          ...
          <b-button variant="outline-secondary" :to="{ name: 'admin.crm.invoices' }">Cancel</b-button>
        </div>
      </b-form>
    </b-card>
  </VerticalLayout>
...
async function submit() {
  ...
  try {
    ...
    if (created?.id != null) {
      router.push({ name: 'admin.crm.invoices.detail', params: { id: String(created.id) } })
    } else {
      router.push({ name: 'admin.crm.invoices' })
    }
  } catch (e: unknown) {

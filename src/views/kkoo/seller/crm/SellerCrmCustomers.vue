  ...
  <VerticalLayout>
    <b-card title="Customers">      <p class="text-muted mb-3">Your CRM customers: name, phone, email, address, credit balance, notes.</p>
      ...
      <b-table v-if="items.length" :items="items" :fields="fields" striped responsive>
        <template #cell(credit_balance)="data">{{ formatCurrency(data.item.credit_balance) }}</template>
        <template #cell(actions)="data">
          <b-button size="sm" variant="outline-primary" :to="{ name: 'seller.crm.customers.detail', params: { id: String(data.item.id) } }">View</b-button>
        </template>
      </b-table>

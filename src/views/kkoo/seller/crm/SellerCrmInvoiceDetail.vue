<template>
  <VerticalLayout>
    <b-button variant="outline-secondary" size="sm" class="mb-3 invoice-no-print" :to="{ name: 'seller.crm.invoices' }">← Back to Invoices</b-button>
    <template v-if="invoice">
    <b-card :title="editing ? 'Edit invoice' : 'Invoice detail'" class="invoice-print-area">
    ...
    <b-card v-else-if="loading" title="Invoice detail"><p class="text-muted">Loading…</p></b-card>
...
async function saveEdit() {
  ...
  try {
    ...
    if (invoice.value) invoice.value = { ...invoice.value, ...payload }
    editing.value = false
  } catch (e: unknown) {

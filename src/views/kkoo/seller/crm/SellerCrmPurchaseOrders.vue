<template>
  <VerticalLayout>
    <b-card title="Purchase Orders">      <p class="text-muted mb-3">Track stock purchased from suppliers. PO status: draft, ordered, received, cancelled.</p>
...
const { items, total, page, pageSize, loading, error, hasPagination, setPage, load } = useCrmList(
  (params) => crmApi.getPurchaseOrders({ ...params, business_id: activeBusinessId.value ?? undefined, status: statusFilter.value || undefined }),
  {}
)
const fields = [
  { key: 'id', label: 'ID' },

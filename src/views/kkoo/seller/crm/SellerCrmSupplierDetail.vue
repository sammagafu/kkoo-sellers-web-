<template>
  <VerticalLayout>
    <b-button variant="outline-secondary" size="sm" class="mb-3" :to="{ name: 'seller.crm.suppliers' }">← Back to Suppliers</b-button>
    <b-card v-if="supplier" :title="editing ? 'Edit supplier' : 'Supplier detail'">
      <template v-if="!editing">
        ...
        <p class="mb-1"><strong>Notes</strong> {{ supplier.notes || '—' }}</p>
      </template>
      <template v-else>
    ...
    <b-card v-else-if="loading" title="Supplier detail"><p class="text-muted">Loading…</p></b-card>
    <b-alert v-else-if="error" variant="danger" show>{{ error }}</b-alert>
    <EmptyState v-else title="Supplier not found" />
  </VerticalLayout>
</template>
...
const supplier = ref<Record<string, unknown> | null>(null)
...
function startEdit() {
  ...
  editForm.value = {
    name: String(supplier.value.name ?? ''),
    phone: String(supplier.value.phone ?? ''),
    email: String(supplier.value.email ?? ''),
    address: String(supplier.value.address ?? ''),
    notes: String(supplier.value.notes ?? ''),
  }
  editErrors.value = {}
  editing.value = true
}
...
async function saveEdit() {
  ...
  try {
    ...
    await crmApi.updateSupplier(id.value, payload)
    supplier.value = { ...supplier.value!, ...payload }
    editing.value = false
  } catch (e: unknown) {
...
async function load() {
  ...
  supplier.value = null
  try {
    const { data } = await crmApi.getSupplier(id.value)
    supplier.value = (data ?? {}) as Record<string, unknown>
  } catch (e: unknown) {
    error.value = (e as { message?: string }).message ?? 'Failed to load supplier'
  } finally {
    loading.value = false
  }
}

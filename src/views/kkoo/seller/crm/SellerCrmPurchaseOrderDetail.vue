<template>
  <VerticalLayout>
    <b-button variant="outline-secondary" size="sm" class="mb-3" :to="{ name: 'seller.crm.purchase-orders' }">← Back to Purchase Orders</b-button>
    <b-card v-if="po" title="Purchase order detail">
      ...
      <template v-if="canReceive">
        ...
        <b-table :items="receiveRows" :fields="receiveFields" striped responsive size="sm" class="mb-2">
          <template #cell(product_id)="data">{{ data.item.product_id ?? '—' }}</template>

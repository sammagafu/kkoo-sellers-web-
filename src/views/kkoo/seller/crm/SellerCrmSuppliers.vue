<template>
  <VerticalLayout>
    <b-card title="Suppliers">      <p class="text-muted mb-3">Supplier contacts: name, phone, email, address.</p>
...
const { items, total, page, pageSize, loading, error, hasPagination, setPage, load } = useCrmList(
  (params) => crmApi.getSuppliers({ ...params, business_id: activeBusinessId.value ?? undefined }),
  {}
)
const fields = [
  { key: 'id', label: 'ID' },
